...
function SerieCard({ serieId }) {
  ...
  const fetchSerieDetails = async () => {
    try {
      const response = await axios.get(
        `https://api.themoviedb.org/3/tv/${serieId}?api_key=${apiKey}&language=fr-FR&append_to_response=credits,videos`
      );
      setSerieDetails({
  ...
  const fetchSeriePoster = async () => {
    try {
      const response = await axios.get(
        `https://api.themoviedb.org/3/tv/${serieId}?api_key=${apiKey}&language=fr-FR`
      );
